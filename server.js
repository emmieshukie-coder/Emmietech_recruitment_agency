...
// ---------- PWA MANIFEST FOR FIREFOX/CHROME INSTALL ----------
...
app.get('/sw.js', (req, res) => {
  ...
  res.send(`
    const CACHE_NAME = 'emmietech-v1.1';
...
const sendEmail = async (to, subject, html) => {
  // Add your email service here - SendGrid, Mailgun, etc
...
<div class="job-grid">
${jobs.rows.map(j => `<div class="job-card"><div class="job-title">${j.title}</div><div class="job-company">${j.company}</div><div class="job-meta"><span>📍 ${j.country}</span>${j.visa_sponsorship? '<span class="badge">Visa Sponsorship</span>' : ''}</div><p>${j.description?.substring(0,120)}...</p><a href="/" class="btn">Login to Apply</a></div>`).join('')}
</div>
</div>
<div class="footer">
<p>© 2026 EmmieTech Global. <a href="/privacy">Privacy</a> | <a href="/about">About</a> | <a href="/">Login</a></p>
</div>
<script>
  ...
  try {
    if (gateway === 'stripe' && session_id) {
      ...
      if (order.rows[0].status === 'pending') {
        ...
        const user = await pool.query('SELECT first_name, last_name, phone FROM candidates WHERE id = $1', [order.rows[0].user_id]);
        await sendEmail(ADMIN_EMAIL, `🔥 Stripe: $30 from ${order.rows[0].user_name}`, `
          <h2>New CV Service Order - $30 Paid via Stripe</h2>
          <p><b>Customer:</b> ${order.rows[0].user_name}</p>
          <p><b>Email:</b> ${order.rows[0].user_email}</p>
          <p><b>WhatsApp:</b> <a href="https://wa.me/${user.rows[0].phone.replace(/[^0-9]/g,'')}">${user.rows[0].phone}</a></p>
          <p><b>Job Target:</b> ${order.rows[0].job_title}</p>
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    else res.json({ error: 'Registration failed' });
  }
});
